...
import time
from Rocog_face.use2 import using
...
if __name__ == '__main__':
    ...
    with torch.no_grad() as grad:
        x = time.time()
        image_file = r"D:\PycharmProjects\MTCNN_data\Rocog_face\face_images\2.jpg"
        detector = Detector()
        ...
        with Image.open(image_file) as img:
            ...
            for box in boxes:
                ...
                if len(lists) == 0:  # 防止列表为空报错
                    ...
                    text = str(lists[-1])
                    print(text)
                    imDraw.text((x1, y1), text, fill=(0, 0, 255), font=font)

            # img.save(r"D:\PycharmProjects\MTCNN_data\save_images\19.jpg")

            y = time.time()
            print("侦测这一张图片所用时间：", y - x)
